    ...
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
      ...
      <div className="grid grid-cols-8 border-b sticky top-0 bg-white z-10">
        ...
        {weekDates.map((day) => (
          <div 
            key={day.date}
            className={`p-4 text-center border-r ${day.isToday ? 'bg-primary-50' : 'bg-gray-50'}`}
          >
      ...
      <div className="overflow-y-auto max-h-[600px]">
        {hours.map((hour) => (
          <div key={hour} className="grid grid-cols-8 border-b hover:bg-gray-50">
            {/* Columna de hora */}
            <div className="p-4 border-r bg-gray-50 text-sm font-semibold text-gray-600">
              {hour}:00
            </div>
            
            {/* Columnas de días */}
            {weekDates.map((day) => {
              const dayAppointments = getAppointmentsByDate(day.date).filter(apt => {
              ...
              return (
                <div 
                  key={`${day.date}-${hour}`}
                  className="p-2 border-r min-h-[80px] cursor-pointer hover:bg-primary-50 transition-colors"
                  onClick={() => onDateChange(day.date, `${hour}:00`)}
                >
                  <div className="space-y-1">
                    {dayAppointments.map((apt) => (
                      <div
                        key={apt.id}
                        className={`text-xs p-2 rounded-lg border-l-4 ${getStatusColor(apt.status)} cursor-pointer hover:shadow-md transition-shadow`}
                        onClick={(e) => {
                          e.stopPropagation();
                          // Aquí se abrirá el modal de detalle
                        }}
                      >
