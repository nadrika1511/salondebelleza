...
import { getClientPackages, updateClientPackageRemaining, createClientPackage } from '../services/clientPackageService';

export const Orders = () => {
  console.log("🔥 Orders component mounting");
  ...
  const handleOpenOrder = async (order) => {
    ...
    if (order.clientId) {
      loadClientPackages(order.clientId);
    }
  };
  ...
  const handleSelectStylist = (serviceIndex, stylistId) => {
    setEditedOrder(prev => {
      ...
      const currentService = newServices[serviceIndex];
      
      // Si hace clic en el mismo estilista → Des-seleccionar
      if (currentService.stylistId === stylistId) {
        ...
        // Limpiar propina de este estilista
        const newTips = { ...prev.tips };
        delete newTips[stylistId];
        
        return { ...prev, services: newServices, tips: newTips };
      }
      
      // Si es diferente → Seleccionar nuevo estilista
  ...
  const handleToggleService = (serviceIndex) => {
    setEditedOrder(prev => {
      ...
      const availablePackage = clientPackages.find(
        cp => cp.serviceId === service.serviceId && cp.remaining > 0
      );
      
      if (!service.selected && availablePackage) {
        newServices[serviceIndex] = {
          ...service,
          selected: true,
          paidWithPackage: true,
          clientPackageId: availablePackage.id,
          packageId: availablePackage.packageId
        };
        console.log(`🎁 Auto-usando paquete: ${availablePackage.packageName} (quedan ${availablePackage.remaining - 1} de ${availablePackage.quantity})`);
      } else {
        newServices[serviceIndex] = {
          ...service,
          selected: !service.selected,
          ...(service.selected && {
            paidWithPackage: false,
  ...
  const loadClientPackages = async (clientId) => {
    try {
      const packages = await getClientPackages(clientId);
      // Ordenar por fecha (más antiguo primero)
      packages.sort((a, b) => new Date(a.purchaseDate) - new Date(b.purchaseDate));
      setClientPackages(packages);
      console.log("📦 Paquetes del cliente cargados:", packages);
    } catch (error) {
  ...
  const getAvailablePackageForService = (serviceId) => {
    // Retorna el paquete más antiguo disponible para este servicio
    return clientPackages.find(cp => cp.serviceId === serviceId && cp.remaining > 0);
  };
  ...
  const handleAddProduct = () => {
    ...
    if (!productToAdd.stylistId) {
      alert('Selecciona el estilista que vendió el producto');
      return;
    }

    const product = products.find(p => p.id === productToAdd.productId);
    const stylist = stylists.find(s => s.id === productToAdd.stylistId);
    ...
    setProductToAdd({ productId: '', stylistId: '', quantity: 1 });
  };
  ...
  const handleAddPackage = () => {
    ...
    if (!packageToAdd.stylistId) {
      alert('Selecciona el estilista que vendió el paquete');
      return;
    }

    const pkg = packages.find(p => p.id === packageToAdd.packageId);
    const stylist = stylists.find(s => s.id === packageToAdd.stylistId);
    ...
    setPackageToAdd({ packageId: '', stylistId: '', quantity: 1 });
  };
  ...
  const calculateTotals = () => {
    ...
    const subtotalServices = (editedOrder.services || [])
      .filter(s => s.selected && !s.paidWithPackage)
  ...
  const handleSaveOrder = async () => {
    try {
      ...
      await updateOrder(editedOrder.id, {
        ...
        packagesSold: editedOrder.packagesSold,
  ...
  const canPayWithPackage = () => {
    ...
    // Verificar si hay servicios seleccionados pagados con paquete
    const hasServicesWithPackage = (editedOrder.services || [])
      .some(s => s.selected && s.paidWithPackage);
    
    // Mostrar SOLO botón "Paquete" si:
    // - El total es 0 (no hay nada que cobrar)
    // - Y hay al menos un servicio pagado con paquete
    // EXCLUYE: venta de paquetes sin servicios (eso se paga con otros métodos)
    return totals.total === 0 && hasServicesWithPackage;
  };
  const handleCloseOrder = () => {
    // Validar que la comanda tenga al menos algo (servicios, productos o paquetes vendidos)
    ...
    if (!hasServices && !hasProducts && !hasPackagesSold) {
  ...
  const consumePackages = async (order) => {
    try {
      const servicesWithPackage = (order.services || [])
        .filter(s => s.paidWithPackage && s.clientPackageId);
      
      console.log("🎁 Consumiendo paquetes:", servicesWithPackage.length);
      
      for (const service of servicesWithPackage) {
        const clientPackage = clientPackages.find(cp => cp.id === service.clientPackageId);
        
        if (clientPackage) {
          const newRemaining = clientPackage.remaining - 1;
          await updateClientPackageRemaining(service.clientPackageId, newRemaining);
          console.log(`✅ Paquete consumido: ${service.serviceName} (quedan ${newRemaining})`);
        }
      }
    } catch (error) {
      console.error("Error consumiendo paquetes:", error);
      throw error;
    }
  };
  ...
  const handleConfirmClose = async (paymentMethod) => {
    try {
      ...
      // Verificar si ya se consumieron paquetes anteriormente
      const alreadyConsumed = editedOrder.packagesConsumed === true;
      console.log("🔍 DEBUG - editedOrder.packagesConsumed:", editedOrder.packagesConsumed);
      ...
      await updateOrder(editedOrder.id, {
        ...
        products: editedOrder.products,
        tips: editedOrder.tips,
        packagesConsumed: true, // Marcar que ya se consumieron
        ...totals
      });
      await closeOrder(editedOrder.id, paymentMethod, userData.id);
      
      // SOLO consumir paquetes si NO se han consumido antes
      if (!alreadyConsumed) {
        await consumePackages(editedOrder);
        console.log("✅ Paquetes consumidos (primera vez)");
      } else {
        console.log("⏭️ Paquetes NO consumidos (ya fueron consumidos antes)");
      }
      
      // Crear clientPackages para los paquetes vendidos (solo primera vez)
      if (!alreadyConsumed) {
        for (const pkg of editedOrder.packagesSold || []) {
          try {
            await createClientPackage(editedOrder.clientId, {
              id: pkg.packageId,
            ...
            console.log("✅ ClientPackage creado para:", pkg.packageName);
          } catch (error) {
            console.error("Error creando clientPackage:", error);
          }
        }
      }
  ...
  const getSelectedStylists = () => {
    console.log("🔍 getSelectedStylists ejecutándose");
    console.log("📦 editedOrder.products:", editedOrder?.products);
    if (editedOrder?.products?.length > 0) console.log("🔎 Primer producto completo:", editedOrder.products[0]);
    console.log("🎁 editedOrder.packagesSold:", editedOrder?.packagesSold);
    if (editedOrder?.packagesSold?.length > 0) console.log("🔎 Primer paquete completo:", editedOrder.packagesSold[0]);
    console.log("⚙️ editedOrder.services:", editedOrder?.services?.filter(s => s.selected));
    console.log("🔍 getSelectedStylists ejecutándose");
    ...
    console.log("👥 Estilistas encontrados:", Array.from(stylistsMap.entries()));
                  ...
                  {(editedOrder.services || []).map((service, index) => (
                    <div key={index} className="grid grid-cols-12 gap-2 p-3 hover:bg-gray-50 transition-colors">
                      ...
                      <div className="col-span-6 flex items-center">
                        ...
                        {/* Alerta de Paquete */}
                        {service.selected && service.paidWithPackage && (() => {
                          const pkg = clientPackages.find(cp => cp.id === service.clientPackageId);
                          return pkg ? (
                            <div className="mt-1 px-3 py-1.5 bg-green-50 border border-green-200 rounded-lg inline-block">
                              <span className="text-xs font-semibold text-green-700">
                                🎁 Usando paquete: {pkg.packageName} (quedan {pkg.remaining - 1} de {pkg.quantity})
                              </span>
                            </div>
                          ) : null;
                        })()}
                      ...
                      <div className="col-span-2 flex items-center justify-end">
                        <span className={service.paidWithPackage ? "font-bold text-green-600 line-through" : "font-bold text-gray-800"}>
                          {service.paidWithPackage ? "🎁 Paquete" : `Q${service.price}`}
                        </span>
                      </div>
                  ...
                  {(editedOrder.products || []).map((product, index) => (
                    <div key={index} className="bg-white p-3 rounded-lg flex justify-between items-center">
                      <div className="flex-1">
                        ...
                        {product.stylistName && (
                          <span className="text-xs text-primary-600 block">Vendido por: {product.stylistName}</span>
                        )} × Q{product.price}</p>
                      </div>
                      <div className="flex items-center gap-3">
                  ...
                  {(editedOrder.packagesSold || []).map((pkg, index) => (
                    <div key={index} className="bg-white p-3 rounded-lg flex justify-between items-center">
                      <div className="flex-1">
                        ...
                        {pkg.stylistName && (
                          <span className="text-xs text-primary-600 block">Vendido por: {pkg.stylistName}</span>
                ...
                <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-4">
                  ...
                  {getSelectedStylists().length === 0 ? (
                    <p className="text-center text-gray-400 py-4">
                      Selecciona servicios, productos o paquetes para agregar propinas
                    </p>
                  ) : (
                ...
                <div className="bg-gradient-to-br from-primary-50 to-pink-50 border-2 border-primary-200 rounded-xl p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-gray-700">
                      ...
                      <span className="font-bold">Q{totals.subtotalPackages}</span>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {canPayWithPackage() ? (
                    <button
                      ...
                      className="col-span-3 bg-pink-100 hover:bg-pink-200 border-2 border-pink-400 text-pink-800 p-4 rounded-xl font-bold transition-all"
                    >
                      🎁 Paquete
                    </button>
                  ) : (
                    <>
                      ...
                      <button
                        onClick={() => handleConfirmClose('credomatic')}
                        className="bg-orange-100 hover:bg-orange-200 border-2 border-orange-400 text-orange-800 p-4 rounded-xl font-bold transition-all"
                      >
                        💳 Credomatic
                      </button>
                      <button
                        onClick={() => handleConfirmClose('transferencia')}
